...
echo "🔧 MacClient Screen Recording Permission Fix Script"
echo "=================================================="
...
clean_xcode_data() {
    ...
    # Remove DerivedData
    rm -rf ~/Library/Developer/Xcode/DerivedData/MacClient-* 2>/dev/null || echo "   No DerivedData found"
...
create_proper_app_bundle() {
    ...
    DERIVED_APP=$(find ~/Library/Developer/Xcode/DerivedData -name "MacClient.app" -type d 2>/dev/null | head -n 1)
    
    if [ ! -z "$DERIVED_APP" ]; then
        echo "🔍 Found app in DerivedData: $DERIVED_APP"
        
        # Copy to Applications
        APPS_PATH="/Applications/MacClient.app"
...
open_system_preferences() {
    ...
    echo "   3. Look for 'MacClient' in the list"
    echo "   4. If not found, click the '+' button and navigate to:"
    echo "      /Applications/MacClient.app"
    echo "   5. Check the box next to MacClient to enable it"
    echo "   6. Restart MacClient"
}
...
main() {
    ...
    if create_proper_app_bundle; then
        ...
        echo "2. 🚀 Launch MacClient from Applications folder (not from Xcode)"
        ...
        echo "      ~/Library/Developer/Xcode/DerivedData/MacClient-*/Build/Products/Debug/MacClient.app"
